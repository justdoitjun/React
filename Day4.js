//this
// this는 자기 자신을 지칭하는 파라미터. 
// 
//************************************************Key-Event******************************************* */
//Key-Event. 
//   <정리>
// (event.key)가 파라미터로 들어올 수 있다. 
...
<script>
    ...
    function goKeyUp(){
        console.log("Key Up");
        x = document.getElementById("fname"); // 객체 가져오기
...
// 잠깐 쓰이는 것은 람다식을 이용해서 메모리를 활용하고자 하는 것이고 이 때 eventListenr라는 메소드를 사용함. 
// 즉, 메모리를 아끼려고 하는 것임. 자바는 이것이 가비지 컬렉터가 해주지만, 가비지 컬레거가 실시간이 아니라서...
// 스프링에선 싱글톤이라는 기능으로 객체는 한번만 쓰게 해줄 수도 있다. 
// 즉 이 지랄하는 이유가 메모리를 아끼려고 하는 것임. 
// 웹사이트는 PC만 있는 것이 아님. 
...
<script>
    //핸들러라는 말을 listener도 handler임. 똑같은 단어임. 
    window.onload = () => {
